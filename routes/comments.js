...
router.post('/:post_id/comments/:id/delete', function(req, res, next) {
    Comments().where('id', req.params.id).del().then(function(results){
      res.redirect('/posts/'+req.params.post_id+'/comments')
    })
  })
router.post('/:post_id/comments/:id/edit', function(req, res, next) {
    Comments().where('id', req.params.id).del().then(function(results){
      res.redirect('/posts/'+req.params.post_id+'/comments')
    })
  })
  ...
  router.post('/:post_id/comments/:id', function(req, res, next) {
    Comments().where('id', req.params.id).update(req.body).then(function(result){
      res.redirect('/posts/'+req.params.post_id+'/comments/')
    })
  })
